(() => {
  ...
  var require_vhtml = __commonJS({
    "node_modules/vhtml/dist/vhtml.js"(exports, module) {
      (function(global2, factory) {
        typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define(factory) : global2.vhtml = factory();
      })(exports, function() {
        "use strict";
  ...
  var waitForRender_default = waitForRender;
  ...
  var upload_default = uploadImage;
  ...
  // src/appendElements/popup.ts
  var import_localforage = __toESM(require_localforage());
  var import_vhtml = __toESM(require_vhtml());

  // src/appendElements/submit.ts
  var submit = () => {
    const nextInner = document.getElementById("__next")?.getElementsByClassName("nextInner")[0];
    if (!nextInner)
      return;
    const button = nextInner.querySelector("section > div > div > div > div > div > div > span > a");
  ...
  var popup = (() => {
    if (document.querySelector(".entry-image-popup-container") !== null)
      return document.querySelector(".entry-image-popup-container");
  ...
  var reloadPopup = () => {
    import_localforage.default.getItem("entry-image-uploads").then((value) => {
      if (value === null)
        return;
      ...
      <div class="bottom">
        ${value.length <= 0 ? html`<h3 class="noimages">이미지가 없습니다.<br />"이미지 추가" 버튼을 클릭해 이미지를 추가해보세요.</h3>` : html`<div class="content">
              ${value.reverse().map((image) => {
      ...
      Array.from(popup.querySelectorAll(".popup .bottom .content .item")).forEach((item) => {
        if (item.dataset.id !== void 0)
          item.addEventListener("click", () => {
            ...
            if (notice)
              notice.textContent = `\uCCA8\uBD80\uB41C \uC774\uBBF8\uC9C0: ${item.dataset.origName}`;
          });
      });
    });
  };
  var togglePopup = (type) => {
    const open = (() => type === void 0 ? !window.isPopupOpen : type === "open")();
    reloadPopup();
    if (open)
      document.querySelector(".entry-image-popup-container").style.display = "flex";
    else
      document.querySelector(".entry-image-popup-container").style.display = "none";
    window.isPopupOpen = open;
  };
  document.documentElement.insertBefore(popup, document.head);

  // src/appendElements/uploadButton.ts
  var import_localforage2 = __toESM(require_localforage());
  var uploadButton = () => {
    const nextInner = document.getElementById("__next")?.getElementsByClassName("nextInner")[0];
    if (!nextInner)
      return;
    const buttonContainer = nextInner.querySelector("section > div > div > div > div > div > div");
    ...
    inputElement.onchange = async (e) => {
      ...
      if (!files || !files[0])
        return alert("\uD30C\uC77C\uC62C\uB9AC\uC148");
      const res = await upload_default(files[0]);
      if (!res.success) {
        console.log(res);
        return alert("\uBB54\uAC00\uC774\uC0C1\uD568... \u3142\u3137\u3142\u3137");
      }
      const data = { id: res.data._id, name: res.data.filename, origName: res.data.name, ext: res.data.imageType };
      const uploads = await import_localforage2.default.getItem("entry-image-uploads");
      if (uploads !== null) {
        await import_localforage2.default.setItem("entry-image-uploads", [...uploads, data]);
        reloadPopup();
      }
    };
    const button = buttonContainer.querySelector("div").cloneNode(true);
  ...
  // src/appendElements/mapPosts.ts
  var mapPosts = async (posts) => {
    const nextInner = document.getElementById("__next")?.getElementsByClassName("nextInner")[0];
    if (!nextInner)
      return;
    await waitForRender_default();
    const postNodeList = Array.from(nextInner.querySelectorAll("section > div > div > div > div > div > ul[class] > li[class]:not([data-content])"));
    if (!postNodeList)
      return;
    const postList = Array.from(postNodeList);
    postList.forEach((post, index) => {
      console.log(Array.from(post.querySelectorAll("div > div"))[1].textContent?.trim());
      if (Array.from(post.querySelectorAll("div > div"))[1].textContent?.trim() !== posts[index].content.trim())
        alert("mapping error!");
      post.dataset.content = posts[index].content;
    });
  };
  var mapPosts_default = mapPosts;

  // src/fetch.ts
  window._fetch = fetch;
  window.fetch = async (...params) => {
    const res = await window._fetch(...params);
    const isSelectEntrystory = JSON.parse(params[1]?.body?.toString() ?? "{}").query.replaceAll("\\n", "\n").trim().startsWith("query SELECT_ENTRYSTORY");
    if (isSelectEntrystory) {
      res._json = res.json;
      res.json = async () => {
        const json = await res._json();
        mapPosts_default(json.data.discussList.list);
        return json;
      };
    }
    return res;
  ...
  // src/index.ts
  var import_localforage3 = __toESM(require_localforage());
  window.addEventListener("load", () => waitForRender_default().then(() => {
    import_localforage3.default.getItem("entry-image-uploads").then((value) => {
      if (value === null)
        import_localforage3.default.setItem("entry-image-uploads", []);
    });
